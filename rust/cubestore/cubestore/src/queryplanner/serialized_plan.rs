...
impl LogicalExtensionCodec for CubeExtensionCodec {
    ...
    fn try_encode(&self, node: &Extension, buf: &mut Vec<u8>) -> datafusion::common::Result<()> {
        ...
        } else if let Some(topk_aggregate) = node.node.as_any().downcast_ref::<ClusterAggregateTopK>() {
            ExtensionNodeSerialized::ClusterAggregateTopK(
                topk_aggregate.to_serialized()?,
            )
        } else {
            todo!("{:?}", node)
        };
