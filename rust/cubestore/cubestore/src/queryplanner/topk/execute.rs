...
use datafusion::physical_plan::{
    DisplayAs, DisplayFormatType, ExecutionMode, ExecutionPlan, ExecutionPlanProperties, Partitioning, PhysicalExpr, PlanProperties, SendableRecordBatchStream
};
...
use super::util::{append_value, create_builder};
...
fn cmp_same_types(l: &ScalarValue, r: &ScalarValue, nulls_first: bool, asc: bool) -> Ordering {
    ...
    let o = match (l, r) {
        ...
        (ScalarValue::Decimal128(Some(l), lprecision, lscale), ScalarValue::Decimal128(Some(r), rprecision, rscale)) => {
            assert_eq!(lprecision, rprecision);
            assert_eq!(lscale, rscale);
            l.cmp(r)
        }
        (ScalarValue::Decimal256(Some(l), lprecision, lscale), ScalarValue::Decimal256(Some(r), rprecision, rscale)) => {
            assert_eq!(lprecision, rprecision);
            assert_eq!(lscale, rscale);
            l.cmp(r)
        }
        ...
        (ScalarValue::TimestampSecond(Some(l), ltz), ScalarValue::TimestampSecond(Some(r), rtz)) => {
            assert_eq!(ltz, rtz);
            l.cmp(r)
        }
        (
        ...
        (ScalarValue::TimestampNanosecond(Some(l), ltz), ScalarValue::TimestampNanosecond(Some(r), rtz)) => {
            assert_eq!(ltz, rtz);
            l.cmp(r)
        }
...
fn create_group_by_values(group_by_keys: &[ArrayRef], row: usize, vec: &mut SmallVec<[GroupByScalar; 2]>) -> Result<(), DataFusionError> {
