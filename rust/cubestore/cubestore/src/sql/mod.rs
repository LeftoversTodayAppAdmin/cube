...
impl SqlServiceImpl {
    ...
    // TODO: Maybe put in a different place?
    ...
    async fn explain(
        ...
        fn extract_worker_plans(
            ...
            if let Some(p) = SqlServiceImpl::find_topmost_cluster_send_exec(p) {
        ...
        let res = match query_plan {
            QueryPlan::Select(serialized, _) => {
                let res = if !analyze {
                    ...
                    if let Some((worker_plans, worker_planning_params)) = extract_worker_plans(&router_plan)? {
                        let worker_futures = worker_plans
                            .into_iter()
                            .map(|(name, plan)| async move {
                                self.cluster
                                    .run_explain_analyze(&name, plan.to_serialized_plan()?, worker_planning_params)
                                    .await
                                    .map(|p| (name, p))
                            })
                            .collect::<Vec<_>>();
            ...
            CubeStoreStatement::System(command) => match command {
                ...
                SystemCommand::PanicWorker => {
                    ...
                    if workers.len() == 0 {
                        ...
                        cluster.run_select(worker, plan, WorkerPlanningParams { worker_partition_count: 1 }).await?;
                    }
                ...
                match logical_plan {
                    QueryPlan::Select(router_plan, _) => {
                        ...
                        let (router_plan, _) = self
                            ...
                            .await?;
                        let worker_planning_params = if let Some(p) = SqlServiceImpl::find_topmost_cluster_send_exec(&router_plan) {
                        ...
                        return Ok(QueryPlans {
                            ...
                            worker: self
                                .query_executor
                                .worker_plan(worker_plan, worker_planning_params, mocked_names, chunk_ids_to_batches, None)
                                .await?
                                .0,
                        });
                    }
